// AUTO-GENERATED snapshot from Enstellar canonical model.
// To update: run `pnpm codegen` in this package (requires Enstellar repo at ../../../../../Enstellar).
// Source: Enstellar/packages/canonical-model/generated/typescript/index.ts

export interface Case {
  caseId: string;
  tenantId: string;
  correlationId: string;
  lob: string;
  program?: string;
  status: "intake" | "completeness_check" | "auto_determination" | "clinical_review" | "pend_rfi" | "approved" | "denied" | "partially_denied" | "adverse_modification" | "withdrawn" | "closed" | "determined" | "voided";
  urgency: "standard" | "expedited" | "concurrent";
  member: Member;
  coverage: Coverage;
  requestingProvider: Provider;
  servicingProvider?: Provider;
  serviceLines: ServiceLine[];
  decisions?: Decision[];
  createdAt: string;
  updatedAt: string;
}

export interface Identifier {
  system: string;
  value: string;
}

export interface Coverage {
  coverageId: string;
  tenantId: string;
  memberId: string;
  planId: string;
  groupId?: string;
  subscriberId: string;
  payerName: string;
  lob: string;
  effectiveDate: string;
  terminationDate?: string;
}

export interface Decision {
  decisionId: string;
  tenantId: string;
  caseId: string;
  outcome: "approved" | "denied" | "partially_denied" | "adverse_modification" | "pending" | "not_required";
  ruleArtifactId: string;
  ruleVersion: string;
  criteriaBranch?: string;
  evidenceRefs: string[];
  humanSignoffRequired: boolean;
  humanSignoffActor?: string;
  humanSignoffAt?: string;
  autoApproved: boolean;
  decidedAt: string;
}

export interface Member {
  memberId: string;
  tenantId: string;
  mrn?: string;
  firstName: string;
  lastName: string;
  dateOfBirth: string;
  gender?: "M" | "F" | "O" | "U";
  identifiers?: Identifier[];
}

export interface Provider {
  providerId: string;
  tenantId: string;
  npi: string;
  name: string;
  specialty?: string;
  organizationName?: string;
  identifiers?: Identifier[];
}

export interface ServiceLine {
  serviceLineId: string;
  tenantId: string;
  sequence: number;
  serviceTypeCode: string;
  procedureCode: string;
  procedureDescription?: string;
  quantity?: number;
  units?: string;
  diagnosisCodes: string[];
  placeOfService?: string;
  requestedStartDate?: string;
  requestedEndDate?: string;
}

// Named type aliases derived from generated interfaces — these stay in sync automatically.
export type CaseStatus = Case['status'];
export type Urgency = Case['urgency'];
export type DecisionOutcome = Decision['outcome'];
