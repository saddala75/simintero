...
for d in services/enstellar-packages/canonical-model services/enstellar-packages/event-contracts ; do
  if [ -d "$d" ]; then echo "C2a INVARIANT VIOLATED: $d should be deleted." >&2; exit 1; fi
done
echo "C2a invariant OK: contract cutover complete."
