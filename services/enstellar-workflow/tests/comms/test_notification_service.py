...
async def test_render_and_dispatch_inserts_log_rows_and_publishes_events(pg_pool):
    ...
    async with pg_pool.acquire() as conn:
        ...
        outbox_count = await conn.fetchval(
            "SELECT COUNT(*) FROM shared.outbox WHERE tenant_id=$1"
            " AND envelope->>'schema_ref'='sim.artifact/NotificationSent/v1'",
            tenant_id,
        )
    assert log_count == 2
...
async def test_notification_body_contains_no_phi(pg_pool):
    ...
    async with pg_pool.acquire() as conn:
        outbox_envelope = await conn.fetchval(
            "SELECT envelope FROM shared.outbox WHERE tenant_id=$1"
            " AND envelope->>'schema_ref'='sim.artifact/NotificationSent/v1'",
            tenant_id,
        )
    import json
    if outbox_envelope:
        env = json.loads(outbox_envelope) if isinstance(outbox_envelope, str) else outbox_envelope
        body_in_outbox = env.get("payload", {}).get("body", "")
